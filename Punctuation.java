 ...
 * 12-11-24
 * Punctuation.java
 * This program takes a paragraph and prints all the words that have punctuation attached to them
 * Utilizing everything we have learned in first sem
 * 
*/
...
public class Punctuation
{
		...
		{
			Punctuation pnc = new Punctuation();
			pnc.searchIt();
		}
...
"and I say, \"Come then, let us go forward together with our united strength.\" ");
			String trimmedParagraph = getPunctuationWords(paragraph);
			printWords(trimmedParagraph);
		}
		
		
		
		public String getPunctuationWords(String paraIn)
		{
			
			boolean runner = true;
			String word = new String("");
			String trimmedPara = new String("");
			int lastIndex = 0;
			...
			for(int i = 0; i<paraIn.length(); i++)
			{
				if(paraIn.charAt(i)==' ')
				{
					word = paraIn.substring(lastIndex, i);
					lastIndex = i + 1;
		...
		public boolean checkForPunctuation(String wordIn)
		{
			...
			{
				if(wordIn.charAt(i)==','||wordIn.charAt(i)=='.'||wordIn.charAt(i)==';'||wordIn.charAt(i)==':'||wordIn.charAt(i)=='!'||wordIn.charAt(i)=='?'||wordIn.charAt(i)=='\''||wordIn.charAt(i)=='('||wordIn.charAt(i)==')'||wordIn.charAt(i)=='"')
				{	hasPunc = true;
				}
				else
					hasPunc = false;
		...
		public void printWords(String trimmedIn)
		{
			System.out.println("\n\n\n");
			System.out.print(trimmedIn);
			System.out.println("\n\n\n");
		}
